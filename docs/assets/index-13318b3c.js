 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */Vn(iI,oI,"app");const aI={apiKey:"AIzaSyCvEW0vLSsZci4y-ZGZbiqG133NXLP8SoU",authDomain:"int305fb004-app.firebaseapp.com",projectId:"int305fb004-app",storageBucket:"int305fb004-app.appspot.com",messagingSenderId:"535643364215",appId:"1:535643364215:web:9f59908c5b86d7214bdf72"};Ah(aI);const Us=Mv(),lI={class:"space-y-1 text-gray-500 list-decimal list-inside dark:text-gray-400"},cI={class:""},uI=["onClick"],hI=["onClick"],fI={__name:"TodoList",props:["todos"],emits:["togle-completed","delete-todo"],setup(e){return(t,n)=>(Er(),Tr("ol",lI,[(Er(!0),Tr(ce,null,hm(e.todos,r=>(Er(),Tr("li",{key:r.title,class:"flex justify-between"},[de("div",{class:fi(["mr-5 w-80 pl-2",r.completed?"text-gray-400 line-through":"text-gray-800"])},$l(r.title),3),de("div",cI,[de("button",{type:"button",onClick:s=>t.$emit("togle-completed",r.id),class:"w-32 text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-5 py-1 me-2 mb-2"},$l(r.completed?"Uncomplete":"Complete"),9,uI),de("button",{type:"button",onClick:s=>t.$emit("delete-todo",r.id),class:"text-gray-100 bg-red-600 border border-gray-300 focus:outline-none hover:bg-red-700 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-5 py-1 me-2 mb-2"},"Delete",8,hI)])]))),128))]))}},dI={class:"w-full px-2"},pI={class:"mb-6 flex"},mI={__name:"TodoInput",emits:["update-todos"],setup(e,{emit:t}){const n=t,r=Hu(""),s=async()=>{await nI(Md(Us,"todos"),{createdAt:sI(),completed:!1,title:r.value}),n("update-todos")};return(i,o)=>(Er(),Tr("div",dI,[de("div",pI,[Zp(de("input",{type:"text","onUpdate:modelValue":o[0]||(o[0]=a=>r.value=a),placeholder:"Enter a new task",class:"w-96 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"},null,512),[[mg,r.value]]),de("button",{type:"button",onClick:s,class:"ml-5 text-white bg-gradient-to-r from-teal-400 via-teal-500 to-teal-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-teal-300 dark:focus:ring-teal-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center"},"Add")])]))}},gI={class:"m-5"},_I=de("h1",{class:"mb-2 px-2 text-2xl font-extrabold text-gray-900 dark:text-white"},"Todo List",-1),yI={__name:"App",setup(e){const t=Hu([]),n=async()=>{const o=Md(Us,"todos"),a=await Zv(Wv(o));t.value=[],a.forEach(l=>{let c=l.data();c.id=l.id,t.value.push(c)})},r=async o=>{const a=fa(Us,"todos",o),c=(await Jv(a)).data();await tI(a,{completed:!c.completed}),await n()},s=async o=>{await eI(fa(Us,"todos",o)),await n()};rh(()=>{n()});const i=()=>{n()};return(o,a)=>(Er(),Tr("main",gI,[_I,we(mI,{onUpdateTodos:i}),we(fI,{todos:t.value,onDeleteTodo:s,onTogleCompleted:r},null,8,["todos"])]))}};yg(yI).mount("#app");
